...
function inferAnswerSheetFromPdfText(text) {
  ...
  const optionChoices = /\bE\b/.test(normalized) ? 'ABCDE' : 'ABCD';
  const qMatches = [...normalized.matchAll(/(?:^|\s)(\d{1,3})\s*[.)-]?/g)];
  const qNumbers = qMatches
    .map((m) => Number(m[1]))
    .filter((n) => Number.isFinite(n) && n >= 1 && n <= 200);

  const uniqueSorted = [...new Set(qNumbers)].sort((a, b) => a - b);
  const contiguous = [];
  for (let i = 0; i < uniqueSorted.length; i += 1) {
    if (i === 0 || uniqueSorted[i] === contiguous[contiguous.length - 1] + 1) {
      contiguous.push(uniqueSorted[i]);
    }
  }

  const detectedMax = contiguous.length ? contiguous[contiguous.length - 1] : 0;
  const total = detectedMax >= 20 ? detectedMax : 35;
  ...
  return {
    ...
    detection: {
      source: 'pdf-text',
      detectedQuestionCount: contiguous.length,
      detectedMaxQuestion: detectedMax,
    },
  };
}

/**
 * Parse a PDF buffer and return template analysis + calibration.
 * Uses text extraction, then infers answer-sheet structure heuristically.
 */
async function parsePdfTemplate(pdfBuffer) {
  let PDFParse;
  try {
    const loaded = require('pdf-parse');
    PDFParse = loaded?.PDFParse || loaded?.default?.PDFParse || loaded?.default;
  } catch {
    throw new Error('pdf-parse package is not installed. Run: npm install pdf-parse');
  }

  if (typeof PDFParse !== 'function') {
    throw new Error('pdf-parse is installed but PDFParse class is unavailable.');
  }

  const parser = new PDFParse({ data: pdfBuffer });
  const result = await parser.getText();
  await parser.destroy();
  const analysis = inferAnswerSheetFromPdfText(result?.text || '');
  const calibration = buildCalibrationFromAnalysis(analysis);
