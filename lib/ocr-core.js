...
function detectChoiceInRow({ raw, width, rowRect, optionsStartX, optionsWidth, calibration, optionBands, optionLabels, imageStats }) {
  ...
  const adaptiveMarkedThreshold = imageStats ? deriveAdaptiveMarkedThreshold(imageStats, calibration.markedThreshold) : calibration.markedThreshold;
  ...
  const optionScores = optionLabels.map((label, index) => {
    ...
    const xMarkDarknessThreshold = Math.max(adaptiveMarkedThreshold * 0.62, 24);
    const isFullFill =
      cellStats.darkRatio >= 0.62
      && cellStats.darkness >= fullFillDarknessThreshold
      && cellStats.darkStdDev <= 62;
    const isXMark =
      cellStats.darkness >= xMarkDarknessThreshold
      && cellStats.darkRatio >= 0.09
      && cellStats.darkRatio <= 0.62
      && cellStats.darkStdDev >= 22;
  ...
  const xCandidates = optionScores
    .filter((option) => option.markType === 'x-mark')
    .sort((a, b) => b.xShapeScore - a.xShapeScore);
  const xBest = xCandidates[0] || null;
  const xSecond = xCandidates[1] || null;
  const hasCrossedOutFullFill = ranked.some((option) => option.markType === 'full-fill' && option.label !== xBest?.label);

  let choice = null;
  let method = 'find-marked';

  if (xBest) {
    const xConfidence = xBest.xShapeScore - (xSecond ? xSecond.xShapeScore : 0);
    if (xBest.darkness >= Math.max(adaptiveMarkedThreshold * 0.62, 24) && xConfidence >= 0.8) {
      choice = xBest.label;
  ...
  if (!choice) {
    choice = marked.darkness > adaptiveMarkedThreshold && confidence > calibration.confidenceGap ? marked.label : null;
  }
