...
function detectChoiceInRow({ raw, width, rowRect, optionsStartX, optionsWidth, calibration, optionBands, optionLabels, imageStats }) {
  ...
  optionScores.forEach((option) => {
    ...
    option.intentScore =
      ...
      - (Math.max(0, option.darkRatio - 0.78) * 70);
  });
  ...
  const xCandidates = optionScores
    .filter((option) => option.markType === 'x-mark' || (option.darkStdDev >= 20 && option.darkRatio >= 0.08 && option.darkRatio <= 0.8))
  ...
  if (xBest) {
    ...
    if (xBest.adjustedDarkness >= Math.max(adaptiveMarkedThreshold * 0.16, 6) && xConfidence >= 1.1) {
      choice = xBest.label;
