...
async function toRawGray(inputBuffer, width = 1200) {
  const normalized = sharp(inputBuffer).rotate().grayscale().normalize();
...
async function buildProcessedPreview(inputBuffer, width = 1200) {
  return sharp(inputBuffer)
    .rotate()
...
async function scanBuffer({ fileBuffer, keyMap, total = 35, lang = 'eng', includeDebug = false, calibration: calibrationInput = DEFAULT_CALIBRATION }) {
  const calibration = sanitizeCalibration(calibrationInput);
  const gray = await toRawGray(fileBuffer);
  ...
  const score = keyMap ? compareAnswers(parsedFromLayout, keyMap) : null;
  let debug = null;

  if (includeDebug) {
    const grayPreview = await buildGrayPreview(gray.data, gray.width, gray.height);
    const processedPreview = await buildProcessedPreview(fileBuffer, gray.width);
  ...
  return {
    ...
    debug,
    calibration,
    answerKey: keyMap ? Object.fromEntries([...keyMap.entries()].sort((a, b) => a[0] - b[0])) : null,
    score,
  };
}
