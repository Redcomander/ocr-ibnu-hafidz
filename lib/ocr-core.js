...
function deriveAdaptiveThreshold(stats) {
  ...
  if (stats.darkRatio > 0.42) {
    threshold -= 8;
  }

  return Math.max(120, Math.min(180, Math.round(threshold)));
}
...
function detectChoiceInRow({ raw, width, rowRect, optionsStartX, optionsWidth, calibration, optionBands, optionLabels }) {
...
function detectAnswersFromSheetLayout(raw, width, height, totalQuestions, calibration, optionChoices = 'ABCDE') {
...
async function scanBuffer({ fileBuffer, keyMap, total = 35, lang = 'eng', includeDebug = false, calibration: calibrationInput = DEFAULT_CALIBRATION, rotation = 0, optionChoices = 'ABCDE' }) {
  ...
  const layoutScan = detectAnswersFromSheetLayout(gray.data, gray.width, gray.height, total, calibration, normalizedOptionChoices);
