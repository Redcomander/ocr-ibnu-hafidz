...
function sanitizeCalibration(input = {}, totalQuestions = 35) {
  ...
  if (Array.isArray(input.blocks)) {
    merged.blocks = merged.blocks.map((block, index) => {
      ...
      return {
        ...
        optionBands: sanitizeBands(source.optionBands ?? block.optionBands, OPTION_LABELS.length, 0.04),
  ...
  merged.blocks = merged.blocks.map((block) => {
    ...
    return {
      ...
      optionBands: sanitizeBands(block.optionBands, OPTION_LABELS.length, 0.04),
...
function detectChoiceInRow({ raw, width, rowRect, optionsStartX, optionsWidth, calibration, optionBands }) {
  const safeOptionBands = sanitizeBands(optionBands, OPTION_LABELS.length, 0.04);
...
function detectAnswersFromSheetLayout(raw, width, height, totalQuestions, calibration) {
  ...
  calibration.blocks.forEach((block) => {
    ...
    const optionBands = sanitizeBands(block.optionBands, OPTION_LABELS.length, 0.04);
...
async function scanBuffer({ fileBuffer, keyMap, total = 35, lang = 'eng', includeDebug = false, calibration: calibrationInput = DEFAULT_CALIBRATION, rotation = 0 }) {
  ...
  const calibration = sanitizeCalibration(calibrationInput, total);
